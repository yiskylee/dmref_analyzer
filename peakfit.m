function [FitResults,LowestError,BestStart,xi,yi,yoffset]=peakfit(signal,center,window,NumPeaks,peakshape,extra,NumTrials,start)
% Version 1.3: April, 13, 2011. A command-line peak fitting program for
...
% 5. yi: matrix containing the y values of model peaks at each xi. 
%    Type plot(xi,yi(1,:)) to plot peak 1 or plot(xi,yi) to plot all peaks
%
% T. C. O'Haver (toh@umd.edu).  Version 1.3: April, 2011. 
%
% Example 1: 
...
warning off all
AUTOZERO=0;
%[DFitResults,DLowestError,DBestStart,Dxi,Dyi,Dyoffset] = peakfit([w(:,i) Icorr(:,i)], 1328.1, 120, 2, 2)
%[FitResults,LowestError,BestStart,xi,yi,yoffset]=peakfit(signal,center,window,NumPeaks,peakshape,extra,NumTrials,start)
...
yoffset = min(Y);  % keep track of this since the problem is rescaled FAH
...
n1=val2ind(X,center-window/2); % A very simple 3-line function that returns the index and the value of the element of vector x that is closest to a particular numerical value.
n2=val2ind(X,center+window/2);
xx=X(n1:n2)-xoffset;
yy=Y(n1:n2); %now for example, we capture the peak area data
...
if AUTOZERO==1,
  ...
  yy=yy-((Y2-Y1)/(X2-X1)*(xx-X1)+Y1); % picked a linear baseline with first 10th of the mean data as the base and rest mean data as the height
end % if
...
n=length(xx); 
newstart=start;
switch NumPeaks % here we used numpeaks as case 2
    case 1
...
for k=1:NumTrials, %here numbtrails is 1
    % disp(k) % prints the current trial number as progress indicator
  switch peakshape %here we are using lorentzian case 2
    case 1
...
A=zeros(NumPeaks,n); % n=length of xx, numpeaks is 2
for m=1:NumPeaks, 
   switch peakshape
    case 1
...
model=PEAKHEIGHTS'*A;  %PEAKHEIGHTS=zeros(1,NumPeaks)inverse so it goes to 2x1;A=zeros(NumPeaks,n) n is length xx to 2xlength xx
...
% disp(['Best of ' num2str(NumTrials) ' trail fits.'] )
...
% Multiplies each row by the corresponding amplitude and adds them up
mmodel=height'*AA; %AA=zeros(NumPeaks,100);height=zeros(1,NumPeaks);height=PEAKHEIGHTS
                   %model=PEAKHEIGHTS'*A;  %PEAKHEIGHTS=zeros(1,NumPeaks)inverse so it goes to 2x1;A=zeros(NumPeaks,n) n is length xx to 2xlength xx
